...
var Trapdoor = preload("res://Trapdoor.tscn")
...
func make_trapdoor():
	var trapdoor = Trapdoor.instance()
	trapdoor.connect("player_interacted",self,"_next_level")
	add_child(trapdoor)
	trapdoor.position = end_room.position
...
func make_rooms():
	...
	for room in $Rooms.get_children():
		if randf() < cull:
			...
			room_positions.append(Vector2(room.position.x,
										  room.position.y))
...
func make_map():
	...
	var bottomright = Map.world_to_map(full_rect.end)+Vector2.ONE
	...
	for room in $Rooms.get_children():
		...
		corridors.append(p)
	#make_doors()
	Map.update_bitmask_region_async(Map.world_to_map(full_rect.position), Map.world_to_map(full_rect.position+full_rect.size))
...
func _next_level():
	if Global.level == 15:
		get_tree().change_scene("res://congratulation.tscn")
	else:
		Global.level += 1
		_restart_level()

func _restart_level():
	$CanvasLayer/LoadingScreen.appear()
	$Mobs.queue_free()
...
func _on_TileMap_bitmask_updated():
	(start_room as Room_).safe = true
	(end_room as Room_).safe = true
	...
	(player as Character).connect("died", self, "if_die") # Если умер - то переключить на сцену выбора
	play_mode = true
